...
interface Hours {
  monday_friday: { label: string; open: string; close: string };
...
server.tool("get_flower_listings", "Get all flower listings", {}, () => {
  const flowers = readJson<Flower[]>("flowers.json");
  return { content: [{ type: "text", text: JSON.stringify(flowers, null, 2) }] };
});

server.tool(
  "add_flower_listing",
  "Add a new flower listing to the site",
  {
...
server.tool(
  "update_flower_listing",
  "Update an existing flower listing by id",
  {
  ...
  ({ id, tag, name, description }) => {
    ...
    if (tag !== undefined) flowers[idx].tag = tag;
    if (name !== undefined) flowers[idx].name = name;
    if (description !== undefined) flowers[idx].description = description;
    writeJson("flowers.json", flowers);
...
server.tool(
  "remove_flower_listing",
  "Remove a flower listing by id",
  { id: z.string().describe("Id of the listing to remove") },
  ({ id }) => {
    const flowers = readJson<Flower[]>("flowers.json");
...
server.tool(
  ...
  { id: z.string().describe("Id of the review to remove") },
  ({ id }) => {
    const reviews = readJson<Review[]>("reviews.json");
...
server.tool("get_hours", "Get current business hours", {}, () => {
  const hours = readJson<Hours>("hours.json");
  return { content: [{ type: "text", text: JSON.stringify(hours, null, 2) }] };
});

server.tool(
  "update_hours",
  "Update business hours for a day group",
  {
